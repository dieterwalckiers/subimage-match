...
const fs = require("fs");
const subImageFoundInImage = require("../src");
...
//     t.throws(() => subImageFoundInImage(bad, arr), err);
//     t.throws(() => subImageFoundInImage(arr, bad), err);
//     t.end();
// });
...
//     t.throws(() => subImageFoundInImage(smallArr, largeArr), err);
//     t.end();
// });
...
//     const matches = subImageFoundInImage(img1, img2);
...
test("Find sub-image match within image", (t) => {
    ...
    const matches = subImageFoundInImage(img, subImg);
    t.ok(matches, "should find match");
    t.end();
});

test("Find sub-image match within image (larger imgs)", (t) => {
    const img = readImage(getTestImgPath("3b"));
